...
def main():
	...
	cmd = 'gzcat {0} '.format(final_bedpe_file) + \
	'| grep -v "chrM" ' + \
	'| gshuf -n {0} --random-source={1} '.format(n_subsampled_reads, final_bedpe_file) + \
	r"""| awk 'BEGIN{OFS="\t"}{print $1,$2,$3,"N","1000",$9}' """ + \
	'| gzip -nc > {0}'.format(subsampled_tagalign_file)
	print cmd
	os.system(cmd)
	# TODO: change gshuf to shuf and gzcat to zcat for unix cluster
